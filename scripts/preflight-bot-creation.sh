...
set -uo pipefail
REFERENCE_BOT="${1:?usage: $0 <existing-healthy-bot-shortname>  (do NOT bake in a default — Dr. Yoo 2026-05-25)}"
...
CANARY_APP=$(curl -sS -G -H "Authorization: Bearer $APPCAT_AT" \
  ...
  --data-urlencode "\$filter=displayName eq '<existing-healthy-bot-display-name>'" \
  | python3 -c "import json,sys;v=json.load(sys.stdin).get('value',[]);print(v[0]['id'] if v else '')" 2>/dev/null)

if [ -z "$CANARY_APP" ]; then
  fail "no canary '<existing-healthy-bot-display-name>' app in catalog — can't test policy gate"
else
